...
use crate::domain::contracts::repository::{Writable, Executor, SqlxExt};
...
pub async fn create_with<'c>(post: &Post, executor: &mut Executor<'c, Writable>) -> Result<()> {
    sqlx::query("INSERT INTO timeline(
            id,
...
#[allow(dead_code)]
pub async fn create_many<'c>(count: usize, executor: &mut Executor<'c, Writable>) -> Result<Vec<Post>> {
